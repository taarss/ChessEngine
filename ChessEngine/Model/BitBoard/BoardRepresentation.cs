...
namespace ChessEngine.Model.BitBoard
{
    public static class BoardRepresentation
    {
		...
		public static Piece.Piece[] TranslateBitBoardToUI(BitBoard bitBoard)
        {
			Piece.Piece[] pieces = new Piece.Piece[64];
            for (int i = 0; i < bitBoard.Square.Length; i++)
            {
				if (bitBoard.Square[i] != 0)
				{
					string binary = Convert.ToString(bitBoard.Square[i], 2).PadLeft(5, '0');
					string pieceType = "";
					switch (binary.Substring(2))
					{
						case "001":
							pieceType = "King";
							break;
					...
					Piece.Piece piece = new(pieceType, binary.Substring(0, 2) == "01" ? true : false);
					pieces[i] = piece;
				}
			}
			return pieces;
