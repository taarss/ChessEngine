...
namespace ChessEngine.Model.BitBoard
{
	...
	public static class Fen
	{
		...
		public static LoadedPositionInfo PositionFromFen(string fen)
		{
			...
			foreach (char symbol in sections[0])
			{
				...
				else
				{
					...
					{
						int pieceColour = (char.IsUpper(symbol)) ? BitPiece.White : BitPiece.Black;
		...
		public static string CurrentFen(BitBoard board)
		{
			string fen = "";
			for (int rank = 7; rank >= 0; rank--)
			{
				...
				for (int file = 0; file < 8; file++)
				{
					...
					if (piece != 0)
					{
						...
						bool isBlack = BitPiece.IsColour(piece, BitPiece.Black);
